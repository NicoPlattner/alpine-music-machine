...
private:
    ...
    void next(int inNumSamples) {
        ...
        while (rubberband->available() < (int)inNumSamples + (int)startDelay) {
            ...
            // R3 can report zero required input while fewer than one SC block
            // is retrievable. Breaking here inserts silent blocks into the
            // output, stretching time again and audibly modulating pitch.
            if (needed == 0) needed = (size_t)inNumSamples;

            int toProcess = (int)std::min(needed, (size_t)maxProcessSize);
            if (!loop) toProcess = std::min(toProcess, (int)bufFrames - playheadPos);
            ...
            inputFinal = !loop && playheadPos + toProcess >= (int)bufFrames;
            rubberband->process((const float *const *)stretchInBuf, toProcess, inputFinal);
